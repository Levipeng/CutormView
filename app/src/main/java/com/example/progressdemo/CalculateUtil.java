...
public class CalculateUtil {
     ...
     * @return
     */
    public static float getDivisionTextMaxWidth(float maxDivisionValue, String unitName) {
        ...
        for (int i = 2; i <= 10; i++) {
            if (maxDivisionValue * 0.1 >= 1) {
                ...
                String text = String.valueOf(bd.multiply(fen).longValue())+unitName;
                float w = textPaint.measureText(text);
                if (w > max) {
                    max = w;
                }
            } else {
                max = textPaint.measureText(String.valueOf(maxDivisionValue * 10)+unitName);
            }
        }
        return max;
    }
